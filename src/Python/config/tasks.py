from ..tools import cli, devices, testers, messengers
...
COMP_SETUP_COMPLETION = 249
UPDATE_DEVICE_ATTR_COMP = 248
UPDATE_DEVICE_ATTR_MCU = 247


# Innate tasks of the testbed

def register_platform(thread_id, pkt):
    ...
    scheduler = messengers.SerialMessenger.get_messenger(thread_id).scheduler
...
def register_tracker(_, pkt):
    ...
    if not devices.DeviceTracker.was_tracker_created(tracker_name):
        ...
        cli.Node(tracker_name, parent_name=root_node.name)
...
def register_device(thread_id, pkt):
    ...
    if tracker is not None:
        tracker.add_devices(device_count, thread_id)


def register_tester(_, pkt):
...
def is_mcu_setup_complete():
    ...
    if len(_mcu_setup_status) == 0:
        return False
    return all(_mcu_setup_status.values())
